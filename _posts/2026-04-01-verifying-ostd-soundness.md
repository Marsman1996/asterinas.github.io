...
**This post marks Phase II.** Not only have we verified *more* functions' individual behavior, but we have successfully proven that the public memory-management API is **sound**.
...
- We quantify over all traces; in practical terms, this means that $C$ can call into OSTD functions in any order, with any arguments that can be constructed by a safe caller.
- UB is undecidable, but by definition, a Verus-verified is well-defined. We aren't looking for individual cases of UB, we are constructively proving that there is some defined behavior for any $C$, which inherently rules out UB.
